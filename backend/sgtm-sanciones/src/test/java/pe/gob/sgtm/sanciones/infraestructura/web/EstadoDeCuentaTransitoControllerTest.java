...
@DisplayName("Capa web — GET /api/v1/transito/estado-cuenta")
class EstadoDeCuentaTransitoControllerTest {

    private final RepositorioDeMentira repositorio = new RepositorioDeMentira();

    private final MockMvc mvc =
            MockMvcBuilders.standaloneSetup(new EstadoDeCuentaTransitoController(repositorio))
    ...
    @Test
    @DisplayName("siempre pide solo lo pendiente")
    void siemprePideSoloLoPendiente() throws Exception {
        mvc.perform(get("/api/v1/transito/estado-cuenta").param("conductor", "12345678"))
                .andReturn();

        assertThat(repositorio.ultimoCriterio.soloPendientes()).isTrue();
        assertThat(repositorio.ultimoCriterio.documentoInfractor()).isEqualTo("12345678");
    }
