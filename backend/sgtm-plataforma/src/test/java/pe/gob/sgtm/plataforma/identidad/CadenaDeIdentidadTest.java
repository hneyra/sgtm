 ...
 *
 * <p>Es la primera prueba que recorre el camino de ARQ-03 §2 de punta a punta. Las que habia
 * verificaban tramos: {@code TenantContextFilterTest} que el claim se traduce a contexto —con un
 * {@code Jwt} puesto a mano en el {@code SecurityContextHolder}—, y {@code
 * AislamientoConElPoolTest} que el contexto llega a la transaccion. Entre las dos quedaba sin
 * verificar el tramo que decide si el sistema es seguro o no atiende a nadie: <b>quien pone ese
 * {@code Jwt} ahi</b>.
 *
 * <p>Por eso el emisor es {@link EmisorDeMentira} y no un {@code JwtDecoder} inyectado: la
 * configuracion —que haya {@code issuer-uri}, que se descubra el emisor, que se traiga el juego de
 * claves, que se valide la firma y el {@code iss}— es exactamente el sitio donde se cometen estos
 * fallos, y un decodificador de mentira se los salta todos.
 *
 * <p>Y el contexto de Spring es el de verdad —{@link SeguridadWeb} y {@link ConfiguracionDeTenant}
 * tal como los carga la aplicacion—, contra PostgreSQL y conectado como {@code sgtm_app}. Un
 * superusuario omite RLS incluso con {@code FORCE ROW LEVEL SECURITY}: la prueba de aislamiento del
 * esquema lo demuestra, y una prueba escrita sobre esa conexion pasaria en verde sin verificar
 * nada.
 *
 * <h2>Como se demuestra que puede fallar</h2>
 *
 * <ul>
 *   <li>Quitando {@code .oauth2ResourceServer(...)} de la cadena: no hay quien valide el token, el
 *       contexto no se puebla y las cuatro pruebas de acceso caen.
 *   <li>Cambiando {@code anyRequest().authenticated()} por {@code permitAll()}: la peticion sin
 *       token llega al controlador.
 *   <li>Cambiando {@code issuer-uri} por {@code jwk-set-uri}: el token del otro emisor pasa.
 *   <li>Devolviendo el {@code SET LOCAL} a {@code SET SESSION}, o conectando como superusuario: la
 *       municipalidad A ve los predios de B.
 * </ul>
 */
...
class CadenaDeIdentidadTest {
    ...
    class Rechazos {
        ...
        void sinToken() throws Exception {
            ...
            assertThat(respuesta.body()).contains("\"codigo\":\"SIN_IDENTIDAD\"");
