...
| Que el stack quepa en su nodo (13 pruebas, sin Pulumi ni clúster) | **También pasó antes de escribirse.** `aplicar-prod` se colgó cuatro veces entre el 25 y el 26 de agosto de 2026, una casi seis horas. La causa: `reservar-recursos-del-nodo.sh` ponía el total de INF-01 §2 —~1 CPU, ~1 GB— en `system-reserved` **y otro tanto** en `kube-reserved`, o sea el doble, y dejaba `vmd120205` en 2 CPU asignables contra los 2 040m que piden los `Deployment`. Un pod que no se puede ubicar no falla —se queda `Pending`— y el `ConfigGroup` lo espera sin error ni registro. Se demuestra con las dos cifras medidas del mismo nodo, con la reserva mal y bien | Rojo con 2 CPU, verde con 3: **el VPS de 8 GB siempre dio de sí**, lo que no daba era lo que quedaba tras la reserva. Y la reserva duplicada quedó **medida y escrita** en INF-02 §4 tres días antes sin que nadie la comparara con lo que el guion decía reservar |
