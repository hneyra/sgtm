...
| El init container de wal-g con la raíz sellada | Ejecutando la imagen real con `--read-only` y **sin** montar `/tmp` | Rojo: `exit 23`, «curl: (23) client returned ERROR on write». Con `/tmp` montado, exit 0 y el binario arranca —«wal-g version v3.0.5»—, que de paso confirma que el `WALG_SHA256` fijado es el del release |
| El esquema de los 60 objetos de cada ambiente | `kubeconform -strict` contra los esquemas de Kubernetes 1.31, escribiendo `readOnlyRootFileSystem` con la `S` de más | Rojo por `additionalProperties`. Valida el esquema, **no** la admisión del API server: no sustituye al trabajo de CI que los aplica de verdad |
