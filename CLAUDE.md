...
| La cadena de identidad, del token firmado a RLS (11 pruebas, con un emisor OIDC de verdad) | Cuatro roturas: sin `oauth2ResourceServer`; con `anyRequest().permitAll()`; con `jwk-set-uri` en vez de `issuer-uri`; y el filtro leyendo `X-Municipalidad-Id` «por comodidad» | 5, 1, 1 y 1 en rojo |
