...
| Estado | **En ejecución.** E-1, E-2, E-3 y E-5 hechos; E-7 con sus dos entregables de código hechos; E-6 esperando la ordenanza; E-4 **aplazado**, ya reevaluado. Ver §0 |
...
| **H-14** | **El cuadro de valores unitarios no se puede cargar todavía, y no por su ámbito.** Dos motivos, los dos escritos en su propio archivo del corpus: (a) `valores-unitarios-2026.md` §1 dice de sí mismo `‹NO CONFIRMADO EN FUENTE OFICIAL›` que sus 9×7 cifras sean las del Anexo I.2 de la RM 277-2025-VIVIENDA —se transcribieron de un cuadro directoral mensual con la fecha de corte del art. 11 del TUO LTM, que es la hipótesis mejor fundada, no una lectura de la RM—; y (b) la RM publica **un cuadro por región** (Costa, Lima/Callao, Sierra, Selva) y el archivo sólo trae Costa. Con ADR-0017 la región no necesita columna: cada región es una **edición**. Lo que falta es cotejar las cifras contra el Anexo I.2 real. **(a) resuelto el 2026-08-28, leyendo el PDF del Anexo I.2 — y el cotejo devolvió que el cuadro publicado era otro**: la norma tiene **3 partidas** (muros y columnas, techos, puertas y ventanas), no las 7 que el archivo transcribía, así que las 63 cifras que había no eran ni las correctas ni de la forma correcta; las 27 reales están ahora transcritas y comprobadas por tres métodos. Lo que impide cargarlo hoy es que la sustitución dejó al archivo en `TRANSCRITO` —le falta la segunda firma de ADR-0007— y sigue **(b)**: solo Costa | `valores-unitarios-2026.md` §1 y §3, y el PDF del Anexo I.2 leído el 2026-08-28 |
| **H-15** | **A `depreciacion` le falta el uso de la edificación, y son cuatro tablas.** El Anexo I del Reglamento Nacional de Tasaciones publica **cuatro** tablas —01 vivienda, 02 tiendas y depósitos, 03 oficinas, 04 salud/industria/educación—, cada una material × estado × antigüedad. La tabla tiene `(material, estado_conservacion, antiguedad_hasta)` y **ninguna columna de uso**: cargar las cuatro hoy haría que `depreciacion_uq` se quedara con la primera y descartara las otras tres en silencio, que es depreciar una oficina con el porcentaje de una vivienda. No se puede resolver con ediciones distintas: una determinación necesita las cuatro a la vez. Es el mismo defecto de forma que la categoría vehicular (H-16), y por eso `PublicarCuadros` rechaza este cuadro nombrando el motivo | `depreciacion.md` §1 (cuatro tablas) contra `V1__nucleo_y_catastro.sql` |
