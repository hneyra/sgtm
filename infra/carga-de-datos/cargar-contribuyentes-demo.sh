#!/usr/bin/env bash
# Siembra un padron de contribuyentes FICTICIO en una instalacion de demostracion (#290),
# corriendo el proceso batch CargarContribuyentesDeDemostracion (backend/sgtm-contribuyentes)
# como un Job de un solo uso.
#
# Mismo patron que cargar-catalogo-vial.sh, con UNA diferencia que no se negocia: el
# proceso pregunta por municipalidad.es_demostracion antes de leer una sola fila, y si la
# municipalidad no esta marcada NO ESCRIBE NADA (SoloEnDemostracion). Este guion no puede
# comprobarlo -no habla con la base-, y por eso la guarda esta dentro del proceso: un
# --municipalidad-id equivocado en un digito metia ocho personas que no existen en el
# padron de una municipalidad que ya opera, y aqui no se borra nada (RNF-051).
#
# El archivo de ejemplo esta en ejemplos/contribuyentes.csv. Va ANTES que
# cargar-fichas-demo.sh: cada ficha nombra a su titular por codigo de contribuyente.
#
#   uso: cargar-contribuyentes-demo.sh --ambiente stg|prod --municipalidad-id N \
#        --archivo ejemplos/contribuyentes.csv [--namespace sgtm-stg] [--observacion "..."]
#
...
ARCHIVO=""
NAMESPACE=""
OBSERVACION="Siembra de contribuyentes ficticios para la demostracion (#290)"
while [ $# -gt 0 ]; do
    case "$1" in
...
[ -n "$ARCHIVO" ] || { echo "Falta --archivo (el CSV de contribuyentes ficticios; hay uno en ejemplos/contribuyentes.csv)." >&2; exit 2; }
[ -f "$ARCHIVO" ] || { echo "No existe el archivo: $ARCHIVO" >&2; exit 2; }
NAMESPACE=${NAMESPACE:-sgtm-$AMBIENTE}

SUFIJO=$(date +%s)
RECURSO="sgtm-${AMBIENTE}-carga-demo-contribuyentes-${SUFIJO}"
...
kubectl -n "$NAMESPACE" create configmap "$RECURSO" --from-file=contribuyentes.csv="$ARCHIVO"
...
metadata:
  ...
  labels:
    proyecto: sgtm
    ambiente: $AMBIENTE
    componente: carga-demo-contribuyentes
spec:
  backoffLimit: 0
  template:
    metadata:
      labels:
        ...
        # un solo uso que necesita hablar con la base. Con "carga-demo-contribuyentes" el pod arranca
    ...
    spec:
      ...
      containers:
        - name: carga-demo-contribuyentes
          image: $IMAGEN
          env:
            ...
            - name: SGTM_DB_CLAVE
              valueFrom:
                secretKeyRef:
                  ...
                  key: clave-app
            - name: SGTM_CARGACONTRIBUYENTESDEMO_MUNICIPALIDADID
              value: "$MUNICIPALIDAD_ID"
            - name: SGTM_CARGACONTRIBUYENTESDEMO_ARCHIVO
              value: /datos/contribuyentes.csv
            - name: SGTM_CARGACONTRIBUYENTESDEMO_USUARIODELPROCESO
              value: carga-demostracion
            - name: SGTM_CARGACONTRIBUYENTESDEMO_OBSERVACION
              value: "$OBSERVACION"
          volumeMounts:
            - name: datos
              mountPath: /datos
