...
# corriendo el proceso batch CargarCajas (backend/sgtm-tesoreria) como un Job de
# un solo uso.
#
...
# configuracion con la que una municipalidad REAL abre su caja. Sin ninguna, la primera
...
ARCHIVO=""
NAMESPACE=""
OBSERVACION="Alta de las ventanillas de la municipalidad (#430)"
while [ $# -gt 0 ]; do
    case "$1" in
...
kubectl -n "$NAMESPACE" create configmap "$RECURSO" --from-file=cajas.csv="$ARCHIVO"
  ...
  template:
    metadata:
      labels:
        ...
        # un solo uso que necesita hablar con la base. Con "carga-cajas" el pod arranca
            ...
            - name: SGTM_DB_CLAVE
              valueFrom:
                secretKeyRef:
                  ...
                  key: clave-app
            - name: SGTM_CARGACAJAS_MUNICIPALIDADID
              value: "$MUNICIPALIDAD_ID"
            - name: SGTM_CARGACAJAS_ARCHIVO
              value: /datos/cajas.csv
            - name: SGTM_CARGACAJAS_USUARIODELPROCESO
              value: carga-cajas
            - name: SGTM_CARGACAJAS_OBSERVACION
              value: "$OBSERVACION"
          volumeMounts:
            - name: datos
              mountPath: /datos
